...
const Navbar = (props: any) => {
  ...
  const actionStatusUrl = (
    ...
    if (status === "DRAFT") {
      return `https://sign-swift-two.vercel.app/user/${userId}/document/${documentId}/step1`;
    } else if (status === "SIGN") {
      return `https://sign-swift-two.vercel.app/user/${userId}/signdoc/${documentId}`;
      ...
      debounce(async (inpstring: string) => {
        try {
          const response = await axios.post(
            "https://sign-swift-two.vercel.app/api/document/searchDocumentForUser",
            {
              userId: user?.id,
              email: user?.email,
